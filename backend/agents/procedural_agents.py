...
from backend.core.utilities import (
    C_ACTION, C_RESET, C_GREEN, C_YELLOW, C_RED, client, LLM_MODEL, generate_content_with_retry
)
...
class CleanQueryAgent:
    ...
    """
    def __init__(self, agent_id: str = "clean_query_agent", model: str = LLM_MODEL):
        self.id = agent_id
        self.model = model
    ...
    def generate_semantic_query(self, query: str) -> str:
        ...
        try:
            ...
            print(f"{C_RED}[{self.id.upper()} ERROR] Gemini generation failed: {e}{C_RESET}")
            return query


#================ CODE DEBUG BLOCK ===============================
if __name__ == "__main__":
    from backend.core.utilities import C_CYAN, C_RESET, C_GREEN, C_YELLOW, C_RED

    print(f"\n{C_CYAN}*** STARTING PROCEDURAL AGENT ISOLATED TESTS ***{C_RESET}")

    # Set project root to parent parent directory
    ...
    try:
        ...
        # Save output to level_0_procedural_output.json in project root
        output_json_file = project_root / "level_0_procedural_output.json"
        with open(output_json_file, "w", encoding="utf-8") as f:
            json.dump(state_after_clean, f, indent=4, ensure_ascii=False)
